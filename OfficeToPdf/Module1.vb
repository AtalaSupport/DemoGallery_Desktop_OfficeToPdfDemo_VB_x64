...
Namespace OfficeToPdf
    ...
    Friend Class Module1
        ...
        Public Shared Sub Main(ByVal args As String())
            ...
            Console.WriteLine("OfficeToPdf starting")

            ' This is how you add support for Office Documents
            ' (.docx, .doc, .rtf, .xls, .xlsx, .pptx, .ppt, .vsdx, .vsd, .msg)
            ' NOTE: if you don't set the resolution property, the default is 96 dpi 
            ' which is often a bit low for many peoples needs
            ' In order for OfficeDecoder to work, you need a license for our Office Addon,
            ' you also need a reference to Atalasoft.dotImage.Office.dll
            ' and you need the using Atalasoft.Imaging.Codec.Ofice; statement up at the top of this app
            ' Finally, you need to make sure that the Perceptive Filter dlls 
            '    ISYS11df.dll
            '    ISYSreaders.dll
            '    ISYSreadershd.dll
            '    Perceptive.DocumentFilters.dll
            ' are all updated / replaced with the ones from 
            ...
            ' for compiling for x64
            ' these perceptive dlls must come from the same version of DotImage as your DotOmage dlls
            RegisteredDecoders.Decoders.Add(New OfficeDecoder() With {.Resolution = 200})

            Dim imgPath As String = GetWorkingDir()
            Dim inFile As String = imgPath & Convert.ToString("target.docx")

            Console.WriteLine("Please select the .docx, .doc, .rtf, .xlsx, .xls, .ppt, .pptx, .msg, .vsd, vsdx  File to convert to PDF:")

            Using dlg As New OpenFileDialog()
                dlg.InitialDirectory = imgPath
                dlg.FileName = Path.GetFileName(inFile)
                dlg.Filter = "MS Word (*.docx;*.doc;*.dot;*.dotx;)|*.docx;*.doc;*.dot;*.dotx;|Rich Text Format (*.rtf)|*.rtf;|Excel Spreadsheet (*.xlsx;*.xls)|*.xlsx;*.xls;|PowerPoint (*.ppsx;*.ppt;*.pptm;*.pptx)|*.ppsx;*.ppt;*.pptm;*.pptx;|Visio (*.vsd;*.vsdm;*.vsdx;*.vdw)|*.vsd;*.vsdm;*.vsdx;*.vdw;|Outlook (*.msg;)|*.msg;|All Files (*.*)|*.*"
            ...
            Console.WriteLine("OfficeToPdf finished... press ENTER to quit")
            Console.ReadLine()
        End Sub


        ''' <summary>
        ''' Given the filename of a supported file type as input and a pdf file as output, convert the
        ''' incoming file to a SingleImageOnly PDF
        ''' 
        ''' This demonstrates the most memory-efficient way to do the conversion.
        ''' </summary>
        ''' <param name="inFile">A .docx file to use as the input</param>
        ...
        Private Shared Sub ConvertToPdf(ByVal inFile As String, ByVal outFile As String)
        ...
        Private Shared Sub DoAboutSplash()
            Console.WriteLine("WordToPDF Demo")
            Console.WriteLine()
            Console.WriteLine("***************************************************************************")
            Console.WriteLine("A very simple console app that converts a Word (.docx or .doc) file")
            Console.WriteLine("into a PDF by using in a memory-efficient way using FileSystemImageSource.")
            Console.WriteLine()
            Console.WriteLine("Keep in mind that our support of the older word document format ")
            Console.WriteLine(".doc (Word 97 - 2007) is limited. It will work for simpler .doc documents, ")
            Console.WriteLine("but for best results, convert your .doc files to the newer ")
            Console.WriteLine(".docx (Word 2010 and newer) format first.")
            Console.WriteLine()
